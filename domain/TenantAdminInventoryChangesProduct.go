...
const (
	...
	ErrTenantAdminInventoryChangesProductTenantNotFound 			= `tenant admin not found`
	ErrTenantAdminInventoryChangesProductInvChangeNotFound 		= `inventory not found` 
	ErrTenantAdminInventoryChangesProductNotTenant						= `must be tenant admin to do this operation`
	ErrTenantAdminInventoryChangesProductProductNotFound			= `product not found`
	ErrTenantAdminInventoryChangesProductSaveFailed      			= `inventory changes save failed`
)

var TenantAdminInventoryChangesProductMeta = zCrud.Meta{
	Fields: []zCrud.Field{
		{
			Name:      mBusiness.Id,
			Label:     "ID",
...
func (d *Domain) TenantAdminInventoryChangesProduct(in *TenantAdminInventoryChangesProductIn) (out TenantAdminInventoryChangesProductOut) {
	...
	user.Id = sess.UserId
	if !user.FindById() {
		out.SetError(fiber.StatusBadRequest, ErrTenantAdminInventoryChangesProductUnauthorized)
		return
	}

	if in.WithMeta {
		out.Meta = &TenantAdminInventoryChangesProductMeta
	}
	...
	case zCrud.CmdUpsert, zCrud.CmdDelete, zCrud.CmdRestore:
		if user.Role != TenantAdminSegment {
			out.SetError(400, ErrTenantAdminInventoryChangesProductNotTenant)
			return
		}
		...
		if invChange.Id > 0 {
			if !invChange.FindById() {
				out.SetError(400, ErrTenantAdminInventoryChangesProductInvChangeNotFound)
				return
			}
			...
			if !product.FindById() {
				out.SetError(400, ErrTenantAdminInventoryChangesProductProductNotFound)
				return
			}
		...
		if !invChange.DoUpsertById() {
			out.SetError(500, ErrTenantAdminInventoryChangesProductSaveFailed)
		}
	...
	case zCrud.CmdList:
		...
		out.InventoryChanges = invChange.FindByPaginationByProduct(&TenantAdminInventoryChangesProductMeta, &in.Pager, &out.Pager)
	}
